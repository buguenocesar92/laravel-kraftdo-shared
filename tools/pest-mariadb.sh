...
CONTENEDOR=${KD_MARIADB_CONTENEDOR:-kraftdo-shared-mariadb-test-$$}
# Puerto 0 = que lo elija el kernel entre los libres; se consulta después con
# `docker port`. Se puede fijar con KD_MARIADB_PORT, pero fijarlo devuelve el
# choque: es para depurar una corrida sola, no para el uso normal.
PUERTO_PEDIDO=${KD_MARIADB_PORT:-0}
IMAGEN=${KD_MARIADB_IMAGE:-mariadb:11}
...
KD_MARIADB_HOST=127.0.0.1 KD_MARIADB_PORT="$PUERTO" \
    vendor/bin/pest "$@"
