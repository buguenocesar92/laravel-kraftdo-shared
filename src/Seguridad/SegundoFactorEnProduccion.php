...
namespace Kraftdo\Shared\Seguridad;
 ...
 * - `enabled` apagado en producción deja el panel —con los datos personales
 *   de cada titular— detrás de una contraseña sola.
 *
...
{
    /**
     * Pares de claves equivalentes: la del sistema suelto y la de kraftdo-acceso.
     *
     * @var list<string>
     */
        ...
        foreach (self::INTERRUPTORES as $clave) {
            ...
            if (config($clave) !== null && config($clave) === false) {
                $problemas[] = "«{$clave}» está apagado con APP_ENV=production: el panel, que muestra "
                    .'los datos personales de cada titular, queda detrás de una contraseña sola.';
            }
        }
